...
const TOOL_STYLES: Record<string, { bg: string; text: string; bgDark: string; textDark: string; pillBg: string; pillText: string; pillBgDark: string; pillTextDark: string }> = {
  ...
  'binary-converter': {
    ...
    pillBgDark: 'bg-green-950/30',
    pillTextDark: 'text-green-300'
  },
  ...
  'url-encoder-decoder': {
    ...
    pillBgDark: 'bg-cyan-950/30',
    pillTextDark: 'text-cyan-300'
  },
  'uuid-generator': {
    bg: 'bg-[#FCE7F3]',
    text: 'text-[#DB2777]',
    bgDark: 'bg-pink-950/40',
    textDark: 'text-pink-400',
    pillBg: 'bg-pink-50',
    pillText: 'text-pink-700',
    pillBgDark: 'bg-pink-950/30',
    pillTextDark: 'text-pink-300'
  },
  'jwt-decoder': {
    bg: 'bg-[#E0E7FF]',
    text: 'text-[#4F46E5]',
    ...
    pillBgDark: 'bg-indigo-950/30',
    pillTextDark: 'text-indigo-300'
  },
  'hash-generator': {
    bg: 'bg-[#F5F5F4]',
    text: 'text-[#57534E]',
    bgDark: 'bg-stone-950/40',
    textDark: 'text-stone-400',
    pillBg: 'bg-stone-100',
    pillText: 'text-stone-700',
    pillBgDark: 'bg-stone-950/30',
    pillTextDark: 'text-stone-300'
  },
  'hex-rgb-converter': {
    bg: 'bg-[#FFEDD5]',
    text: 'text-[#F97316]',
...
export default function HomeView({ onSelectTool, showAllTools }: HomeViewProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [recentToolIds, setRecentToolIds] = useState<string[]>([]);
  const visibleTools = useMemo(() => (showAllTools ? TOOLS : TOOLS.slice(0, 10)), [showAllTools]);
  ...
  const filteredTools = useMemo(() => {
    if (!normalizedQuery) return visibleTools;
  ...
  }, [normalizedQuery, visibleTools]);

  const recentTools = useMemo(
    () => TOOLS.filter((tool) => recentToolIds.includes(tool.id)),
    [recentToolIds]
  );

  const handleSearchSubmit = () => {
    if (normalizedQuery) {
      const match = visibleTools.find(
        (tool) =>
          tool.name.toLowerCase().includes(normalizedQuery) ||
          tool.description.toLowerCase().includes(normalizedQuery) ||
          tool.keywords.some((k) => k.toLowerCase().includes(normalizedQuery))
  ...
  return (
    <div className="w-full bg-[#FAFBFD]">
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <div className="grid grid-cols-3 gap-3 max-w-sm sm:max-w-md mx-auto mt-12 bg-white/60 backdrop-blur-xs p-4 rounded-2xl border border-rose-100/50 shadow-xs">
            <div className="text-center">
              <div className="text-xl md:text-2xl font-black text-[#FF334B] font-sans">{TOOLS.length}+</div>
      ...
      {!showAllTools && <AdComponent slot="homepage-top" />}

      {/* 2. RECENTLY USED SECTION */}
      {!showAllTools && recentTools.length > 0 && (
      ...
      <section id="tools-grid-anchor" className={`${showAllTools ? 'py-10 md:py-12' : 'py-12'} mx-auto max-w-7xl px-4 sm:px-6 lg:px-8`}>
        <div className="border-b border-rose-100 pb-4 mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight font-sans">
              {showAllTools ? 'All Tools Directory' : 'Dynamic Products Directory'}
            </h2>
            <p className="text-xs font-semibold text-slate-500 mt-1 font-sans">
              {showAllTools ? 'All available utilities are instantly loaded on client side. No credentials needed.' : 'Featured tools are ready instantly. Open the full directory to view every utility.'}
            </p>
          </div>

          <span className="text-[11px] font-bold text-[#FF334B] font-mono bg-rose-50 p-1 px-3 rounded-full">
            Showing {filteredTools.length} of {visibleTools.length} utilities
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
          {filteredTools.map((tool) => {
            ...
            return (
              <div
                key={tool.id}
                className="tool-card group relative flex min-h-[315px] flex-col justify-between bg-white rounded-2xl border border-slate-100 p-6 shadow-xs hover:border-[#FF334B] hover:shadow-[0_8px_20px_-8px_rgba(255,51,75,0.18)] transition-[border-color,box-shadow] duration-150 cursor-pointer"
                onClick={() => onSelectTool(tool.id)}
              >
                <div>
                  {/* Decorative icon box with exact Vibrant Theme color coding */}
                  <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${styles?.bg || 'bg-rose-50'} ${styles?.text || 'text-[#FF334B]'}`}>
          ...
          >
            <div>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-rose-100/50 text-[#FF334B]">
                <svg className="w-5 h-5 font-black" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              ...
              <p className="mt-1.5 text-xs text-slate-500 leading-relaxed font-sans font-medium">
                Open the complete ToolHub directory with calculators, converters, encoders, generators, and developer utilities.
              </p>
            </div>

            <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
              <span className="pill text-[10px] uppercase font-bold px-3 py-1 rounded-full bg-rose-50 text-[#FF334B] font-mono">
                {TOOLS.length} Tools
              </span>
              <span className="inline-flex items-center gap-1 text-xs font-bold text-[#FF334B]">
                View All
          ...
          {showAllTools && (
            <div className="tool-card group relative flex min-h-[315px] flex-col justify-between bg-rose-50/15 rounded-2xl border-2 border-dashed border-rose-200/65 p-6 select-none">
              <div>
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-rose-100/50 text-[#FF334B]">
      ...
      {!showAllTools && <AdComponent slot="homepage-middle" />}

      {/* 4. BENEFITS */}
      {!showAllTools && <BenefitsSection />}

      {/* 5. FAQS */}
      {!showAllTools && <FAQSection />}

      {/* Floor placement ad */}
      {!showAllTools && <AdComponent slot="homepage-bottom" />}
    </div>
  );
}
